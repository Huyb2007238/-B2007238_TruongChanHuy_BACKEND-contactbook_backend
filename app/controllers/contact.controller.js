...
exports.findOne = async(req, res, next ) => {
    try {
        ...
        return next(
            new ApiError(
                500,
                `Error retreving contact with id=${req.params.id}`
            )
        );
    }
};
...
exports.delete = async(req, res,next ) => {
    try{
        ...
        return next(
            new ApiError(
                500,
                `Could not deleted contact with id=${req.pargams.id}`
            )
        );
    }
};
...
exports.deleteAll = async(_req, res,next ) => {
    try {
        ...
        return next (
            new ApiError(500,"An error occurred while remobing all contacts")
        );
    }
};
